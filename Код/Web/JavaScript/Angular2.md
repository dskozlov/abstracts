...
- [Введение](#Введение)
- [Организация структуры проекта](#Организация-структуры-проекта)
- [Директивы](#Директивы)
  - [Компоненты](#Компоненты)
  - [Структурные директивы](#Структурные-директивы)
    - [Создание собственной структурной директивы](#Создание-собственной-структурной-директивы)
  - [Атрибутные директивы](#Атрибутные-директивы)
    - [Создание собственной атрибутной директивы](#Создание-собственной-атрибутной-директивы)
- [Связка данных (Data Binding)](#Связка-данных-data-binding)
  - [Непосредственно в шаблоне (String Interpolation)](#Непосредственно-в-шаблоне-string-interpolation)
    - [Pipes](#pipes)
      - [Создание пайпа](#Создание-пайпа)
  - [По атрибуту (Property Binding)](#По-атрибуту-property-binding)
...
- [Модели](#Модели)
- [Сервисы и внедрение зависимостей (Dependency Injection)](#Сервисы-и-внедрение-зависимостей-dependency-injection)
- [HTTP запросы](#http-запросы)
  - [Асинхронная загрузка данных](#Асинхронная-загрузка-данных)
  - [Обработка ошибок](#Обработка-ошибок)
- [Пути (Routes)](#Пути-routes)
  - [Параметризация пути](#Параметризация-пути)
    - [Запросы по параметру](#Запросы-по-параметру)
  - [Дочерние пути](#Дочерние-пути)
- [Работа с формами](#Работа-с-формами)
  - [Управление из шаблона](#Управление-из-шаблона)
    - [Валидация формы](#Валидация-формы)
  - [Управление из класса компоненты](#Управление-из-класса-компоненты)
    - [Массивы данных формы](#Массивы-данных-формы)
    - [Упрощённый вариант](#Упрощённый-вариант)
    - [Создание собственного валидатора](#Создание-собственного-валидатора)
      - [Создание асинхронного валидатора](#Создание-асинхронного-валидатора)
    - [Отслеживание изменений формы](#Отслеживание-изменений-формы)
...
После этого для компоненты становится доступным событие.
```html
<compName (clicked)="onClicked($event)"></compName>
```

Теперь в родительской компоненте можно обработать новое событие clicked
```ts
onClicked(value) {
  alert(value);
}
...
ng new <app-name> # в папке app-name
  ...
  --directory (-dir) <dir-name> # создается проект app-name в папке dir-name

ng init # в текущей папке
  --name <app-name> # создается проект app-name в текущей папке
```

Как только приложение будет создано, его можно будет скомпилировать и запустить.
```sh
ng build # разовая компиляция
  --production (-prod) # компиляция для выкатки на сервер

ng serve # компиляция, создание сервера и отслеживание изменений в проекте
  -p 7777 # номер порта (по умолчанию 4200)
```
...
ng g
  ...
  class (cl)
  module (m)
  route compName --lazy false # создаёт компоненту compName и путь к ней
```
...
Для того, чтобы пользоваться сторонними библиотеками, их следует подключить в файле `.angular-cli.json`.
```json
"scripts": [
...
После этого в файле, в котором нужно использовать функционал библиотеки необходимо перед декоратором прописать следующее:
```ts
declare var $: any; // теперь можно пользоваться jQuery
```

### Использование препроцессоров CSS

В Angular 2 доступны препроцессоры CSS такие как SASS/SCSS, Less, Stylus.
Пользоваться можно любым, однако для активации следует указать название компилятора CSS в терминале при генерации новой компоненты `ng g c compName --style=scss`, либо в файле настроек `.angular-cli.json`.
```json
"styleExt": "scss"
```
