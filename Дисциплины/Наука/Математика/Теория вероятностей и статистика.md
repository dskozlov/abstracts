...
[Экспоненциальное](https://ru.wikipedia.org/wiki/Экспоненциальное_распределение), $Exp(\lambda)$ | $\begin{cases}\lambda\exp(-\lambda x), & x \geq 0 \\\\ 0, & x < 0 \end{cases}$ | $\frac1{\lambda}$ | $\frac1{\lambda^2}$
